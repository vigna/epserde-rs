...
pub struct MemCase<'a, S: DeserializeInner>(pub(crate) DeserType<'a, S>, pub(crate) MemBackend)
where
    Self: 'a;
...
impl<'a, S: DeserializeInner> MemCase<'a, S> {
    ...
    pub fn encase(s: S) -> MemCase<'a, EncaseWrapper<S>> {
    ...
    pub fn uncase(&self) -> &DeserType<'a, S> {
...
impl<'a, 'b, S: DeserializeInner> IntoIterator for &'a MemCase<'b, S>
where
    for<'c, 'd> &'c DeserType<'d, S>: IntoIterator,
{
    type Item = <&'a DeserType<'b, S> as IntoIterator>::Item;
    type IntoIter = <&'a DeserType<'b, S> as IntoIterator>::IntoIter;
...
impl<'a, A, S: DeserializeInner> AsRef<A> for MemCase<'a, S>
where
    for<'b> DeserType<'b, S>: AsRef<A>,
{
    fn as_ref(&self) -> &A {
        self.uncase().as_ref()
    }
...
impl<'a, S: DeserializeInner> std::ops::Deref for MemCase<'a, S> {
    type Target = DeserType<'a, S>;
...
impl<'a, Idx, S: DeserializeInner> core::ops::Index<Idx> for MemCase<'a, S>
where
    for<'b> DeserType<'b, S>: core::ops::Index<Idx>,
{
    type Output = <DeserType<'a, S> as core::ops::Index<Idx>>::Output;
...
impl<'a, 'b, S: DeserializeInner, T: DeserializeInner> PartialEq<MemCase<'a, T>> for MemCase<'b, S>
where
    for<'c, 'd> DeserType<'c, S>: PartialEq<DeserType<'d, T>>,
{
    fn eq(&self, other: &MemCase<T>) -> bool {
        self.uncase().eq(other.uncase())
    }
}

impl<'a, S: DeserializeInner> Eq for MemCase<'a, S> where
    for<'c, 'd> DeserType<'c, S>: PartialEq<DeserType<'d, S>>
{
}
/*
impl<'a, S: DeserializeInner> PartialOrd<MemCase<'a, S>> for MemCase<'a, S>
where
    for<'c> DeserType<'c, S>: PartialOrd<DeserType<'c, S>>,
{
...
impl<'a, S: DeserializeInner> Ord for MemCase<'a, S>
where
    DeserType<'a, S>: Ord,
    for<'c> DeserType<'c, S>: PartialOrd<DeserType<'c, S>>,
{
