...
/// constant; when it is false the constant is simply `false`. Either way the
/// per-field expansion is emitted exactly once, rather than both in the
/// constant and in the assertion.
fn gen_zero_copy_exprs(
    is_repr_c: bool,
...
fn reject_epserde_attrs(attrs: &[syn::Attribute], msg: &str) -> syn::Result<()> {
    for attr in attrs {
        if attr.meta.path().is_ident("epserde") {
...
fn gen_epserde_enum_impl(ctx: &EpserdeContext, e: &syn::DataEnum) -> proc_macro2::TokenStream {
    ...
    let (is_zero_copy_expr, could_be_zero_copy) =
        gen_zero_copy_exprs(ctx.is_repr_c, &all_fields_types);
    let (mut ser_where_clause, mut deser_where_clause) =
        ...
        Data::Enum(e) => e.variants.iter().try_for_each(|v| {
            reject_epserde_attrs(
                ...
                "#[epserde(...)] attributes are not supported on enum variants; \
                 place field-level markers such as force_full_copy on the variant's fields",
            )?;
...
pub fn type_info_derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    // The attributes below only shape the (de)serialization types and bounds
    // generated by derive(Epserde); derive(TypeInfo) generates none of them.
    // Reject them instead of silently ignoring them. Note that this check
    // lives in the standalone entry point: the Epserde derive validates these
    // attributes itself and calls type_info_derive_impl directly.
